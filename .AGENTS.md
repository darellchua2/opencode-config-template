...
#### OpenCode Tooling
- `skill*`, `agent*`, `auditor*`, `maintainer*` → `opencode-tooling-subagent`
- Examples: "create new skill", "audit skills", "create agent"

#### Additional Quality Patterns
- `coverage*`, `badge*` → `documentation-subagent` (for coverage-readme-workflow)
- `dry*`, `refactor*duplicate*` → `linting-subagent` (for typescript-dry-principle)
...
| `explore` | `read` | None | None | Fast codebase exploration, read-only access |
