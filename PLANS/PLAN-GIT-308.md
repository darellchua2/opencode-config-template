...
- [ ] **3.1** Update `deploy/setup.sh` + `setup.ps1` help-text/banner (NOT counts)
...
- [ ] **3.5** Regenerate `deploy/registry.json` via `node deploy/build-registry.mjs`
...
- [ ] **3.6** Add `docling-mcp-skill` edge to `deploy/dependency-map.json`
...
- [ ] **3.7** Invoke `documentation-sync-workflow-skill` (or delegate to `opencode-tooling-subagent`)
