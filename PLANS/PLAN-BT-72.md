...
This is **Phase 1 only**. It establishes the discovery (customer) + requirements (internal) subagents and the shared rendering skill. BRD and TDD are Phase 2.
...
- **Skills**: +2 (vision-creation-skill, interactive-document-rendering-skill), 1 renamed (prd-creation-skill → srs-creation-skill). Source count 108 → 110.
...
| `skills/srs-creation-skill/SKILL.md` (renamed from prd-creation-skill) | — | requirements-specialist-subagent, ticket-creation-subagent (permit + dir convention) | high |
...
- [ ] **2.2** Rename `prd-specialist-subagent` → `requirements-specialist-subagent`
    - **Why:** Subagent now owns internal SRS (IEEE 830) only; name must reflect SRS not PRD; rename preserves git history
    - **Done when:** `git mv opencode_app/.opencode/agents/prd-specialist-subagent.md opencode_app/.opencode/agents/requirements-specialist-subagent.md` executed; description rewritten (triggers: "create srs", "software requirements", "functional spec", "specification", plus back-compat "create prd"/"product requirement"→route to SRS); purpose scoped to internal-only (NO customer mode); permission block updated (`skill: srs-creation-skill, interactive-document-rendering-skill, domain-modeling-skill, grilling-skill, docx-creation-skill, xlsx-specialist-skill: allow`; `task: image-analyzer-subagent allow, xlsx-specialist-subagent allow, explore allow, "*": deny`); self-detect doc-type workflow note (Phase 2 adds BRD; for now SRS only); output path `docs/srs/`; Return Contract updated
...
- [ ] **3.1** Update `ticket-creation-subagent.md`
    ...
    - **Done when:** L25 `prd-creation-skill: allow` → `srs-creation-skill: allow`; L147 skill table description updated; L147/217/221/224/231 all `docs/prd/` → `docs/srs/`, `PRD-draft` → `SRS-draft`, `PRD-{key}` → `SRS-{key}`, `PRD_PATH` → `SRS_PATH`, `**PRD**:` → `**SRS**:`; the `prd-creation-skill` row in "Skills Used" table → `srs-creation-skill`
...
- [ ] **3.2** Update `ticket-plan-workflow-skill/SKILL.md`
    ...
    - **Done when:** L255 `**PRD**: $PRD_PATH ... docs/prd/` → `**SRS**: $SRS_PATH ... docs/srs/` (variable name + path)
...
- [ ] **4.1** Update `README.md`
    ...
    - **Done when:** Skill table (~L286) entry `prd-creation-skill` → `srs-creation-skill`, add `vision-creation-skill` + `interactive-document-rendering-skill`, recount Framework/Document skills (108→110); Subagent table (~L329) row `prd-specialist-subagent` → `requirements-specialist-subagent`, add `discovery-specialist-subagent` row
...
- [ ] **4.5** Mirror all setup.sh changes in `deploy/setup.ps1`
    ...
    - **Done when:** All changes from 4.4 mirrored in PowerShell syntax at ~L376, ~L388, ~L1177, ~L1249, ~L1748
    - **Consumers affected:** Windows users

- [ ] **4.6** Verify/update `opencode_app/README.md`
    - **Why:** Docker docs may reference prd; must not break
    - **Done when:** Grep `opencode_app/README.md` for `prd`; if found, update; if absent, no-op (verify only)
    - **Consumers affected:** Docker users

### Phase 5: Verification

- [ ] **5.1** Grep-confirm zero stale `prd` references in source
    - **Why:** Any leftover `prd-specialist` / `prd-creation-skill` / `docs/prd` will cause routing failures or broken deploy scripts
    - **Done when:** `grep -rn "prd-specialist\|prd-creation-skill\|docs/prd\|PRD-draft" opencode_app/ deploy/ README.md AGENTS.md` returns zero matches EXCEPT `CHANGELOG.md` history rows (which are intentionally preserved)
...
- [ ] **5.3** Verify agent/skill counts are internally consistent
    ...
    - **Done when:** `ls opencode_app/.opencode/agents/ | wc -l` == 35; `find opencode_app/.opencode/skills -name SKILL.md | wc -l` == 110; setup.sh + setup.ps1 + README all reflect these
...
- [ ] `srs-creation-skill/` exists (renamed from prd-creation-skill, IEEE 830 structure, `docs/srs/`, xlsx clause)
- [ ] `discovery-specialist-subagent.md` exists (glm-5-turbo, customer-facing, permits vision+rendering+wireframer+domain-modeling+grilling+docx+xlsx+pptx skills, task image-analyzer+xlsx allow)
- [ ] `requirements-specialist-subagent.md` exists (renamed from prd-specialist, glm-5-turbo, internal, permits srs+rendering+domain-modeling+grilling+docx+xlsx skills, task image-analyzer+xlsx allow)
- [ ] `ticket-creation-subagent.md` references `docs/srs/` + `srs-creation-skill` (zero `docs/prd`)
- [ ] `ticket-plan-workflow-skill/SKILL.md` uses `**SRS**:` + `docs/srs/` in PLAN header
- [ ] README.md, AGENTS.md, deploy/.AGENTS.md, setup.sh, setup.ps1 reflect new agent + 2 skills + rename (counts consistent: 35 agents / 110 skills)
- [ ] Zero stale `prd-specialist` / `prd-creation-skill` / `docs/prd` in source (CHANGELOG history excluded)
...
| Count mismatch (banner says 38 agents but source ls says 34) | high | med | Step 5.3 reconciles; treat source ls as authoritative for the +1/+2 delta |
| ticket-creation-subagent edits missed (multi-line) | med | high | Step 3.1 enumerates all 6 line groups; 5.1 grep catches stragglers |
...
- TDD subagent (Phase 2) will be `glm-5.1` (sound-reasoning), unlike the 5-turbo document subagents — different model tier justifies its own subagent
...
- **BRD**: create `brd-creation-skill` (BABOK standard), add to `requirements-specialist-subagent` permissions + self-detect (no new agent)
- **TDD**: create `tdd-specialist-subagent` (`glm-5.1`, sound-reasoning) + `tdd-creation-skill`; wire CodeGraph/architecture tools; separate subagent justified by model tier + design-authoring workflow

---

*Created by ticket-creation-subagent full workflow — BT-72.*
