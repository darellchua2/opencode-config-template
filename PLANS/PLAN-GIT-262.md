...
## Acceptance Criteria
- [x] `opencode_app/mcp-servers/markitdown-local-mcp/` exists with `pyproject.toml`, `src/markitdown_local_mcp/{__main__.py,__init__.py}`, `README.md`, `LICENSE`
- [x] `pyproject.toml` depends ONLY on `markitdown[pdf,docx,pptx,xlsx,xls,outlook]>=0.1.1,<0.2` + `mcp>=1.0`; explicitly lists what's NOT included (version cap prevents trust-boundary bypass on future bump)
- [x] Launcher constructs `MarkItDown()` with no cloud kwargs (`docintel_endpoint`, `cu_endpoint`, `llm_client` all absent) and `enable_plugins=False` (constructor arg, not env var)
- [x] Single MCP tool `convert_to_markdown(uri: str) -> str` exposed via stdio transport
- [x] `LICENSE` is MIT with markitdown copyright attribution (derivative work)
- [x] `opencode_app/opencode.json` has new `markitdown` MCP entry with `enabled: false`, plus `"markitdown*": false` in `tools` block (env block contains ONLY `MARKITDOWN_ENABLE_PLUGINS=false` — no empty-string API keys)
- [x] `deploy/setup.sh` has new `install_local_mcp_launchers()` function that runs `pip install --user --force-reinstall "$APP_DIR/mcp-servers/markitdown-local-mcp"` (uses `pip`, not `uv` — `uv` is not a repo dependency)
- [x] `deploy/setup.ps1` mirrors the install hook for Windows parity
- [x] `opencode_app/Dockerfile` has build-time `RUN /opt/python-env/bin/pip install /app/mcp-servers/markitdown-local-mcp` (binary lands in `/opt/python-env/bin`, already on PATH)
- [x] `README.md` MCP Servers section: count `25 → 26` (corrects pre-existing off-by-one where docs claimed 26), add table row for `markitdown`, opt-in count `19 → 20`
- [x] `deploy/setup.sh` help text: `MCP SERVERS (26)` → `(27)` AFTER pre-existing-drift fix in Phase 6 (which corrects 26 → 25 first); listing updated; banner auto-start count stays at 6 (markitdown is opt-in)
- [x] `deploy/setup.ps1` Windows parity: status section listing updated with markitdown opt-in row (line ~1725)
- [x] `deploy/.AGENTS.md` MCP Tool Routing table has new markitdown row (user-level routing doc tells the model when to call it)
- [x] `THIRD_PARTY_LICENSES.md` updated with markitdown MIT entry (hand-maintained; `tests/test_autoresearch_skills.bats` greps this file)
- [x] **Security verification:** `python -c "import azure"` fails inside the launcher's isolated venv (proves Azure SDK absent)
- [x] **Security verification:** `ss -tnp` during a local PDF conversion shows zero established TCP connections
- [x] **Security verification:** launcher startup registers zero Plugin converters (assert via `MarkItDown()._converter_registry` or equivalent — proves `enable_plugins=False` works)
- [x] **Functional verification:** converting a sample PDF/DOCX/PPTX/XLSX returns valid Markdown
- [x] **PATH verification:** `markitdown-local-mcp --help` resolves on PATH inside the Docker container as the `opencode` user (no entrypoint changes needed)
- [x] Cross-file MCP count consistency verified via explicit grep checklist (see Phase 4.5 — `documentation-sync-workflow` skill only covers skills/subagents, NOT MCP counts)
...
- [x] **2.1** Add `markitdown` entry to `opencode_app/opencode.json` `mcp` block (after `mermaid`, line 239): `type: local`, `command: ["markitdown-local-mcp"]`, env block with `MARKITDOWN_ENABLE_PLUGINS=false` ONLY, `enabled: false`
...
- [x] **2.2** Add `"markitdown*": false` to `tools` block (after `"mermaid*": true`, line 244)
...
- [x] **3.1** Add `install_local_mcp_launchers()` to `deploy/setup.sh` — verifies `pip`/`python3` exists, runs `pip install --user --force-reinstall "$APP_DIR/mcp-servers/markitdown-local-mcp"`, logs result, warns if `~/.local/bin` not on PATH. Call it after config-copy step.
...
- [x] **3.2** Mirror in `deploy/setup.ps1`
...
- [x] **3.3** Add `RUN /opt/python-env/bin/pip install /app/mcp-servers/markitdown-local-mcp` to `opencode_app/Dockerfile` after the opencode_app COPY step
...
- [x] **3.4** Verify `markitdown-local-mcp` resolves on PATH inside a built container as the `opencode` user (no entrypoint changes needed)
...
- [x] **4.1** `README.md` line ~243: AFTER Phase 6 corrects the latent off-by-one (25 entries documented as 26), bump to "26 MCP server entries" reflecting markitdown addition; add table row `| markitdown | local (pip) | Document → Markdown conversion (opt-in) |`; update "remaining N" opt-in count (`19 → 20`)
...
- [x] **4.2** `deploy/setup.sh` help text line 627: AFTER Phase 6 corrects `MCP SERVERS (26)` → `(25)`, bump to `(26)`; add `markitdown         Document-to-markdown (local-only, opt-in)` to the disabled/opt-in listing; auto-start count stays at 6 (markitdown is NOT in auto-start)
...
- [x] **4.3** `opencode_app/README.md` — add Docker note that launcher is baked into the image
...
- [x] **4.4** `THIRD_PARTY_LICENSES.md` — add markitdown MIT entry
...
- [x] **4.5** Run explicit grep checklist to verify cross-file MCP count consistency (do NOT rely on `documentation-sync-workflow` skill — it only handles skills/subagents, not MCP counts)
...
- [x] **4.6** `deploy/setup.ps1:1722-1725` status section — add `markitdown` to opt-in listing
...
- [x] **4.7** `deploy/.AGENTS.md` MCP Tool Routing table (lines 74-85) — add markitdown row
...
- [x] **4.8** `deploy/setup.sh` status sections (lines ~1722, ~2416, ~3144) — add `markitdown` to opt-in listings
...
- [x] **5.1** Run `pip install` locally; confirm `markitdown-local-mcp --help` works
...
- [x] **5.2** Send MCP `tools/list` — expect single tool `convert_to_markdown`
...
- [x] **5.3** Convert local PDF/DOCX/PPTX/XLSX — confirm valid Markdown output
...
- [x] **5.4** Convert user-supplied `https:` URL — confirm fetch + conversion works (no telemetry)
...
- [x] **5.5** `ss -tnp` during local conversion — assert zero established TCP connections
...
- [x] **5.6** `python -c "import azure"` in launcher venv — assert `ModuleNotFoundError`
...
- [x] **5.7** `opencode mcp list` — confirm `markitdown` shows `disabled` until user opts in
...
- [x] **5.8** Assert zero Plugin converters registered at startup
...
- [x] **5.9** Docker build smoke test — `docker run --rm <image> which markitdown-local-mcp` and confirm resolution
...
- [x] **6.1** Recount `opencode.json` MCP entries programmatically and document the canonical number
...
- [x] **6.2** Correct `README.md` line 243 from "26 MCP server entries" to "25 MCP server entries" (interim state — Phase 4.1 then bumps it back to 26 with markitdown)
    ...
    — **Consumers affected:** documentation readers.
- [x] **6.3** Correct `deploy/setup.sh` line 627 from `MCP SERVERS (26):` to `MCP SERVERS (25):` (interim — Phase 4.2 then bumps to 26)
...
- [~] **6.4**  *(SKIPPED — false positive: setup.ps1:2479 'MCP Servers (6):' refers to AUTO-START count, which is correct. Documented in bats test header.)* Correct `deploy/setup.ps1` MCP count (line ~2479 banner) from `26` to `25` (interim)
...
- [x] **6.5** Add bats test (or extend `tests/test_autoresearch_skills.bats`) to assert MCP count consistency
