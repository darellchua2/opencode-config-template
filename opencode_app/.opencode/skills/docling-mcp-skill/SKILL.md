...
This installs `docling-mcp[local]` (the MCP server wrapper) and clears `mcp.servers.docling.disabled` + flips the root `permissions` rule `{ "action": "docling*", "effect": "allow" }` in `opencode.json`. After an opencode restart, `docling*` tools register and are callable directly.
