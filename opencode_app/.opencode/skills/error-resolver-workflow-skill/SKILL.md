...
2. **Fallback — `zai-vision-analysis-skill`**: direct Z.AI vision API call (`glm-5v-turbo` — a different model from the native one) via bash, for text-only sessions or when the vision provider is not connected.
