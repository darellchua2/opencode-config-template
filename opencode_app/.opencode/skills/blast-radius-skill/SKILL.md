...
<!--
  Vendored from cursor/plugins (pstack) @ 60c641e4fad6 (MIT)
  Source: https://github.com/cursor/plugins/blob/60c641e4fad674784b30abcf9f8915dea39df38d/pstack/skills/blast-radius/SKILL.md
  Pinned at commit 60c641e4fad6. Re-vendor deliberately on upstream bumps.
  Sibling refs (how/why/arena) neutralized — not vendored in this repo.
...
1. Read the change. The diff, the symbols it adds, changes, and deletes, and what it now does differently, including the part the diff doesn't spell out. Pull the PR and commits (`gh pr view`, `git log`).
...
4. Be honest about each risk. Give it a real chance of happening and a real cost if it does. Keep the risks you confirmed; list the ones you checked and cleared separately. Same standard of evidence. Cite a real `file:line`, a search that finds nothing is still an answer, and never make up a caller or an API.
...
6. For a big or wide change, get a second independent review. Ask another model or reviewer the same question and merge the answers. Different models catch different real bugs.
