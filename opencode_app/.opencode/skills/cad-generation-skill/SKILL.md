---
name: cad-generation-skill
description: >-
  Create, modify, inspect, validate STEP-first parametric CAD parts and
  assemblies from specs, reference images, or 2D drawings; STEP/STL/3MF/GLB
  outputs.
license: Apache-2.0
...
Also use it when the user asks for STL, 3MF, or native GLB output from CAD geometry. Keep those workflows secondary and load `supported-exports.md` for details. For 2D DXF drawings, use the `the cad-dxf-skill (load via skill tool)` skill; when a DXF projects from a 3D part, this skill owns the STEP geometry and `the cad-dxf-skill (load via skill tool)` owns the drawing.
...
4. **Check named purchasable components.** When an assembly includes named off-the-shelf actuators, servos, motors, electronics boards, connectors, or other purchasable components, search `the cad-step-parts-skill (load via skill tool)` before creating simplified placeholder geometry. If no exact match is found, record the miss and then use a documented envelope.
...
Final responses should include generated files, returned `the cad-viewer-skill (load via skill tool)` viewer links, verification snapshots, validation actually run, assumptions, and caveats. Use `references/inspection-and-validation.md` for report structure.
