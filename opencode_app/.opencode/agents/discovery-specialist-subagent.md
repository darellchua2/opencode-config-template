...
This subagent produces **Vision Documents only** (`docs/vision/VISION-{slug}.md`) — the customer-facing artifact for client sign-off. It does **NOT** author internal SRS/BRD (that is `requirements-specialist-subagent`) and does **NOT** create tickets or PLANs (that is `ticket-creation-skill` + `worktree-pipeline-skill`).
...
4. **Optional**: distill into a customer presentation deck via `pptx-specialist-subagent` (delegate via Task tool — it routes to pptx-generate-slide-skill with a user-supplied template) — ask the user if they want this
...
**Reading client-provided source docs (briefs, decks, vision inputs as PDF/DOCX/PPTX):** follow the AGENTS.md → Office Document Extraction Routing rule (markitdown → docling → image-analyzer → pdf-specialist).
