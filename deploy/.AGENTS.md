...
| Document → Markdown conversion (opt-in) | `markitdown` (local-only; PDF/DOCX/PPTX/XLSX/MSG + image EXIF) | Built-in `Read` for plain text | Privacy-hardened launcher at `opencode_app/mcp-servers/markitdown-local-mcp/`. **No cloud/telemetry paths** — structurally excludes Azure SDKs, Google Speech, YouTube API. User-supplied `http:`/`https:` URIs trigger a single `requests.get()` (no Microsoft endpoints — equivalent to `webfetch`). `enable_plugins=False` hard-coded. **Proactive load:** when an agent encounters a `.docx`/`.pptx`/`.xlsx`/born-digital `.pdf` and needs text extraction, load `markitdown-mcp-skill` for the decision tree (markitdown vs `pdf-specialist-skill` vs `image-analyzer-subagent` vs `pdftotext`). |
...
- Prefer atomic commits: one logical change per commit. Split multi-concern work into separate commits (e.g., migration, ORM model, schema, logic, tests).
- Wrap commit body lines at 72 characters. Projects may override this (e.g., 120 chars via commitlint config).
- Never mix style/formatting changes with logic changes in the same commit.
- When in doubt, refer to `git-semantic-commits-skill` for detailed granularity guidance.